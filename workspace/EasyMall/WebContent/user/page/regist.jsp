...
<div class="parent">
  <div class="container">
    <div class="panel rt">
      <form id="fm-register" method="post" action="/EasyMall/regist">
        <div class="txt">
          <p id="newuser">新用户注册
            <span>
        ...
        <div class="form-group">
          ...
          <input required type="password" minlength="6" maxlength="12" placeholder="请再次输入密码" name="repwd" autofocus id="repwd"/>
          <span class="msg-default hidden">密码长度在6到12位之间</span>
        </div>
        <div class="form-group">
...
<script>
  ...
  $('#uname').blur(function(){
    if(this.validity.valueMissing){
      ...
      $.ajax({
		  type:'post', // 请求方式
		  data:'uname='+data, // 请求参数
		  url:'/EasyMall/checkUsername', // 请求路径
		  success:function(data){ // 请求正确响应后的处理逻辑，data是响应数据
	          console.log('开始处理响应数据');
	          if(data=='yes'){
	        	$('#uname')[0].nextElementSibling.innerHTML = '该用户名已被占用';
	        	$('#uname')[0].nextElementSibling.className = 'msg-error';
	          }else{
	        	$('#uname')[0].nextElementSibling.innerHTML = '该用户名可以使用';
	        	$('#uname')[0].nextElementSibling.className = 'msg-success';
	          }
		  }
	  });
    }
  ...
  function checkPwd(){
	  if($('#repwd').val()!=$('#upwd').val()){
    	  $('#repwd')[0].nextElementSibling.innerHTML = '两次密码不一致';
    	  $('#repwd')[0].nextElementSibling.className = 'msg-error';
      }else {
    	  $('#repwd')[0].nextElementSibling.innerHTML = '两次密码一致';
    	  $('#repwd')[0].nextElementSibling.className = 'msg-success';
      }
  }
